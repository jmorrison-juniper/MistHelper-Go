...
func (s *Server) processChannelRequests(ch gossh.Channel, requests <-chan *gossh.Request, sessionID string) {
	for req := range requests { // Iterate SSH channel requests (exec, shell, pty-req, env, etc.)
		switch req.Type {
		case "pty-req": // PTY allocation request -- accept so clients get interactive terminal behaviour
			if req.WantReply { // Acknowledge the PTY request to the client
				if err := req.Reply(true, nil); err != nil { // Accept PTY; the SSH channel already acts as a bidirectional stream
					slog.Error("pty-req reply failed", "session", sessionID, "error", err) // Log so operators can trace reply failures
				}
			}
			slog.Debug("PTY request accepted", "session", sessionID) // Log that we granted the PTY for audit
		case "window-change": // Terminal resize event -- accept silently (we don't use terminal dimensions)
			if req.WantReply { // Some clients expect a reply to window-change
				if err := req.Reply(true, nil); err != nil { // Accept the resize notification
					slog.Error("window-change reply failed", "session", sessionID, "error", err) // Log reply failures
				}
			}
		case "exec", "shell": // The client wants to run the menu (ForceCommand pattern)
		...
		default: // Unknown request types -- deny gracefully per RFC 4254 §5.4
			if req.WantReply { // Reply false to unsupported requests
			...
			slog.Debug("rejected unsupported request", "type", req.Type, "session", sessionID) // Log rejected requests for audit
		}
	}
}
